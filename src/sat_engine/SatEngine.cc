			 ...
			 Val3 detect)
{
  ...
  if ( detect == kVal0 ) {
    for (ymuint i = 0; i < npo; ++ i) {
      ...
      SatLiteral dlit(fval_cnf.dvar(node));
      add_clause(~dlit);
    }
    if ( dom_node != nullptr ) {
      SatLiteral dlit(fval_cnf.dvar(dom_node));
      add_clause(~dlit);
    }
  }
...
void
SatEngine::add_eq_clause(SatVarId var1,
			 SatVarId var2)
{
  SatLiteral lit1(var1);
  SatLiteral lit2(var2);
...
// @param[in] var1, var2 変数番号
void
SatEngine::add_diff_clause(SatVarId var1,
			   SatVarId var2)
{
  SatLiteral lit1(var1);
  SatLiteral lit2(var2);
			    ...
			    const VidMap& fvar_map)
{
  ...
  if ( fault->is_output_fault() ) {
	...
	continue;
      ...
      SatVarId ivar = gvar_map(inode);
      ivars.push_back(ivar);
      ilits.push_back(SatLiteral(ivar));
    }

    SatVarId ofvar = fvar_map(node);
    SatLiteral oglit(gvar_map(node));
    SatLiteral oflit(ofvar);
