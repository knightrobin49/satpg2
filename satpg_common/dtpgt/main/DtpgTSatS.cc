...
DtpgEngine*
new_DtpgTSatS(const string& sat_type,
	      ...
	      UntestOp& uop)
{
  return new DtpgTSatS(sat_type, sat_option, sat_outp, bt, dop, uop);
}

// @brief コンストラクタ
DtpgTSatS::DtpgTSatS(const string& sat_type,
		     const string& sat_option,
		     ostream* sat_outp,
		     BackTracer2& bt,
		     DetectOp2& dop,
		     UntestOp& uop) :
  DtpgTSat(sat_type, sat_option, sat_outp, bt, dop, uop)
{
}
...
void
DtpgTSatS::run(TpgNetwork& network,
	       FaultMgr& fmgr,
	       Fsim& fsim,
...
void
DtpgTSatS::run_single(const TpgFault* fault)
{
  cnf_begin();
