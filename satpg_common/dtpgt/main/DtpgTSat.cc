...
// @brief コンストラクタ
DtpgTSat::DtpgTSat(const string& sat_type,
...
// @brief オプションを設定する．
void
DtpgTSat::set_option(const string& option_str)
{
...
void
DtpgTSat::get_stats(DtpgStats& stats) const
{
  stats = mStats;
}
...
// @breif 時間計測を制御する．
void
DtpgTSat::timer_enable(bool enable)
{
  mTimerEnable = enable;
}
...
// @brief 一つの SAT問題を解く．
SatBool3
DtpgTSat::solve(SatSolver& solver,
	       ...
	       const VidMap& fvar_map)
{
  ...
  if ( ans == kB3True ) {
    ...
    // パタンが求まった．
    ModelValMap2 val_map(hvar_map, gvar_map, fvar_map, model);
