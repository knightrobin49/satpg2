...
void
Bt2Just2::set_max_id(ymuint max_id)
{
  Bt2JustBase::set_max_id(max_id);
  mMaxId = max_id;
  mJustArray.clear();
...
void
Bt2Just2::run(const TpgNode* fnode,
...
void
Bt2Just2::clear_justified_hook(const TpgNode* node)
{
...
Bt2Just2::NodeList*
Bt2Just2::justify(const TpgNode* node,
		  const ValMap2& val_map)
{
...
Bt2Just2::NodeList*
Bt2Just2::just_sub1(const TpgNode* node,
		    const ValMap2& val_map)
{
...
// @param[in] val 値
Bt2Just2::NodeList*
Bt2Just2::just_sub2(const TpgNode* node,
		    const ValMap2& val_map,
		    Val3 val)
{
  ymuint ni = node->fanin_num();
...
// @note mJustifiedMmark がついたノードは mJustifiedNodeList に格納される．
Bt2Just2::NodeList*
Bt2Just2::justify0(const TpgNode* node,
		   const ValMap2& val_map)
{
...
// @param[in] val_map SATの値の割り当て結果を収めた配列
Bt2Just2::NodeList*
Bt2Just2::just0_sub1(const TpgNode* node,
		     const ValMap2& val_map)
{
...
// @param[in] val 値
Bt2Just2::NodeList*
Bt2Just2::just0_sub2(const TpgNode* node,
		     const ValMap2& val_map,
		     Val3 val)
{
  ymuint ni = node->fanin_num();
...
// @brief 新しいリストのセルを返す．
Bt2Just2::NodeList*
Bt2Just2::new_list_cell(const TpgNode* node,
			int time)
{
...
// @brief リストをマージする．
void
Bt2Just2::list_merge(NodeList*& dst_list,
		     NodeList* src_list)
{
...
// @brief リストのサイズを返す．
ymuint
Bt2Just2::list_size(NodeList* node_list)
{
  ymuint n = 0;
...
// @brief リストを削除する．
void
Bt2Just2::list_free(NodeList* node_list)
{
...
void
Bt2Just2::mark_tfo(const TpgNode* node)
{
  if ( mTfoMark[node->id()] ) {
    return;
  }
