...
new_FsimT2()
{
  return new nsFsimT2::FsimT2();
}

END_NAMESPACE_YM_SATPG


BEGIN_NAMESPACE_YM_SATPG_FSIMT2
...
void
FsimT2::set_network(const TpgNetwork& network)
{
  clear();
...
void
FsimT2::set_skip(const TpgFault* f)
{
...
// スキップマークは消される．
void
FsimT2::set_faults(const vector<const TpgFault*>& fault_list)
{
...
// @param[in] op 検出した時に起動されるファンクタオブジェクト
void
FsimT2::sppfp(const NodeVal2List& assign_list,
	      FsimOp& op)
{
  ymuint npi = mNetwork->input_num();
...
void
FsimT2::ppsfp(const vector<TestVector2*>& tv_array,
	      FsimOp& op)
{
  ymuint npi = mNetwork->input_num();
...
// @retval false 故障の検出が行えなかった．
bool
FsimT2::spsfp(const NodeVal2List& assign_list,
	      const TpgFault* f)
{
  ymuint npi = mNetwork->input_num();
...
// @retval false 故障の検出が行えなかった．
bool
FsimT2::_spsfp(const TpgFault* f)
{
  // 正常値の計算を行う．
  for (vector<SimNode*>::iterator q = mLogicArray.begin();
       q != mLogicArray.end(); ++ q) {
...
PackedVal
FsimT2::ffr_simulate(SimFFR* ffr)
{
...
SimNode*
FsimT2::make_node(GateType type,
...
SimNode*
FsimT2::find_simnode(const TpgNode* node) const
{
...
// @param[in] tv テストベクタ
ymuint
FsimT2::calc_wsa(TestVector2* tv)
{
  ymuint npi = mNetwork->input_num();
